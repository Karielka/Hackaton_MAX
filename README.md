...
go mod download
```
3. Запустить базу данных через Docker Compose
```sh
docker compose up -d
```
4. Убедиться, что контейнеры поднялись
```sh
docker ps
```
5. Запустить Go-приложение локально
...
| Найдено несколько вариантов | Бот просит уточнить название.                                |
